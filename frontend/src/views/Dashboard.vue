  ...
  <div class="page">
    ...
    <el-card class="radar" style="margin-top: 16px">
      <template #header>
        <div class="head">
          <span>客户雷达</span>
          <span class="t">点一行打开会话。优先排风险、待回复、报价后沉默和未兑承诺。</span>
        </div>
      </template>
      <el-table :data="rows" height="100%" empty-text="当前范围没有客户会话" @row-click="openRow">
        <el-table-column label="客户" min-width="140">
          <template #default="{ row }">
            <div>{{ row.contact }}</div>
            <div v-if="row.role && row.role !== '—'" class="t">{{ row.role }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="标记" min-width="200">
          <template #default="{ row }">
            ...
            <el-tag v-if="row.quoted" type="warning" size="small" style="margin-left: 4px">报价后沉默</el-tag>
...
const cards = computed(() => [
  { key: "", label: "客户", value: summary.value.customers ?? 0 },
  { key: "pending", label: "待回复", value: summary.value.pending ?? 0, accent: true },
  { key: "quoted", label: "报价后沉默", value: summary.value.quoted ?? 0 },
